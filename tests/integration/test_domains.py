...
async def test_domain_onboarding_and_dns_instructions(client: AsyncClient):
    ...
    login = await client.post(
        "/api/v1/auth/login", json={"email": "domainowner@company.com", "password": "SecurePassword123!"}
    )
...
async def test_custom_domains_can_be_disabled_server_wide(client: AsyncClient):
    ...
    login = await client.post(
        "/api/v1/auth/login", json={"email": "domaindisabled@company.com", "password": "SecurePassword123!"}
    )
    ...
    try:
        res = await client.post(
            "/api/v1/domains", json={"domain": "blocked.example.com", "workspace_id": workspace_id}, headers=headers
        )
