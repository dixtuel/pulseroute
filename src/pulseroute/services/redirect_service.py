    ...
    async def resolve_and_track(
        ...
        if redis_cli:
            try:
                ...
                if cached_json == "NULL":
                    return None, 404, "Link not found (cached)"
                if cached_json:
        ...
        if not link_data:
            ...
            if domain_name:
                query = query.join(CustomDomain, ShortLink.domain_id == CustomDomain.id).where(CustomDomain.domain == domain_name)
            else:
        ...
        # 8. Async Event Ingestion into Redis Stream
        if redis_cli:
            try:
                country_code, _country_name, city = lookup_ip_location(client_ip)
                event_payload = {
